...
if ($type === 'product') {
    $order_id = (int)($_POST['order_id'] ?? 0);
        ...
        $stmt = $pdo->prepare('SELECT id, produto_id AS product_id, nome_produto AS product_name, quantidade AS quantity, pedido_por AS requester_id, estado AS status, devolucao_obrigatoria AS return_required, criado_em AS created_at FROM pedidos WHERE id = ?');
    ...
    $stmt = $pdo->prepare('SELECT id, sala_id, horario_id, pedido_por AS requester_id, estado AS status FROM requisicao_sala WHERE id = ?');
    ...
    $stmt = $pdo->prepare('UPDATE requisicao_sala SET estado = ? WHERE id = ?');
